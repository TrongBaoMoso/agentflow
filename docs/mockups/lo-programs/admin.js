/* Admin console behaviour.
   ...
   Everything below is client-side against window.APPLICANTS so the mockup
   demonstrates the real interactions — tab switch, search, filter, sort,
   promote/demote, approve/decline, bulk actions, drawer — without a backend. */

;(() => {
  const DATA = (window.APPLICANTS || []).map((r) => ({ ...r }))
  ...
  const state = { prog: 'all', q: '', status: '', tier: '', age: '', sort: 'submitted', dir: -1, page: 1 }
  ...
  const inProgram = (r) => state.prog === 'all' || r.program === state.prog
  ...
  const renderKpis = () => {
    ...
    const budget = rows
      .filter((r) => r.status === 'approved' && r.program === 'ambassador')
    ...
    $('[data-kpi="budget"]').textContent = '$' + budget.toLocaleString('en-US')

    $$('[data-seg-count]').forEach((el) => {
      const p = el.dataset.segCount
      el.textContent = p === 'all' ? DATA.length : DATA.filter((r) => r.program === p).length
    })
  }
  ...
  const renderSpark = () => {
    ...
    const rows = DATA.filter(inProgram)
    $('[data-legend="ambassador"]').textContent = rows.filter((r) => r.program === 'ambassador').length
    $('[data-legend="recruiter"]').textContent = rows.filter((r) => r.program === 'recruiter').length
  }
  ...
  const renderDist = () => {
    ...
    if (state.prog === 'recruiter') {
      ...
      groups = [
        { name: 'Ambassador program', n: approved.filter((r) => r.program === 'ambassador').length, cls: '' },
        { name: 'Recruiter program', n: approved.filter((r) => r.program === 'recruiter').length, cls: 'bar__fill--3' }
      ]
      $('[data-dist-title]').textContent = 'Approved by program'
    }
  ...
  const renderTable = () => {
    ...
    $('[data-rows]').innerHTML = rows
      .map(
        ...
        <td><span class="prog prog--${r.program}">${r.program === 'ambassador' ? 'Ambassador' : 'Recruiter'}</span></td>
        <td>
          <span class="lvl">
  ...
  const openDrawer = (id) => {
    ...
    $('[data-dr-dl]').innerHTML = `
      <div><dt>Program</dt><dd>${r.program === 'ambassador' ? 'Ambassador' : 'Recruiter'}</dd></div>
  ...
  $$('.seg').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      state.page = 1
      picked.clear()
      $$('.seg').forEach((b) => b.setAttribute('aria-selected', String(b === btn)))
      renderTierFilter()
      renderAll()
    })
  })
