...
4. **Audit dùng platform `audit-log-service`** (phát hiện 05/08 khi đọc repo: write-once, Pub/Sub → Postgres 16 partitioned + BigQuery archive + digest ký KMS, LIVE staging, tera-be producer đã chạy — recruiting-be copy producer pattern outbox+poller của tera-be, epic tera-be#17). Outbox local đảm bảo độ bền nếu prod rollout audit-svc (#80) chưa xong. Phạm vi audit: sửa offer, đổi comp band, cấp quyền, đổi config, đổi trạng thái bonus — mọi mutation ngoài pipeline. *Trình tự chốt 05/08 (Bao gật):* V001 tạo bảng `audit_outbox` ngay từ đầu; việc nối Pub/Sub → audit-log-service là task riêng tách biệt (làm trong v1 nếu suôn sẻ, lùi v1.1 cũng không sao) — schema không đổi dù nối lúc nào, và outbox tự nó đã là bản ghi audit đọc được tại chỗ.
